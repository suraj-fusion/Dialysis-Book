...
export function SessionsContextProvider({ children }: SessionsProviderProps) {
  const [sessions, setSessionsState] = useState<DialysisSession[]>(mockDialysisSessions.reverse());

  console.log(sessions)

  function addSession(sessionData: DialysisSession) {
    setSessionsState((prevSessions) => [...prevSessions,{id:sessions.length+1, ...sessionData}]);
  }
